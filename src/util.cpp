...
const char * const BITCOIN_CONF_FILENAME = "stone.conf";
const char * const BITCOIN_PID_FILENAME = "stoned.pid";
...
bool LogAcceptCategory(const char* category)
{
    if (category != NULL)
    {
        ...
        if (ptrCategory.get() == NULL)
        {
            ...
            // "stone" is a composite category enabling all Proton-related debug output
            if(ptrCategory->count(string("stone"))) {
...
boost::filesystem::path GetDefaultDataDir()
{
    ...
    // Windows < Vista: C:\Documents and Settings\Username\Application Data\StoneCore
    // Windows >= Vista: C:\Users\Username\AppData\Roaming\StoneCore
    // Mac: ~/Library/Application Support/StoneCore
    // Unix: ~/.stonecore
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "StoneCore";
#else
    fs::path pathRet;
...
#ifdef MAC_OSX
    // Mac
    return pathRet / "Library/Application Support/StoneCore";
#else
    // Unix
    return pathRet / ".stonecore";
#endif
#endif
}
                    ...
                    map<string, vector<string> >& mapMultiSettingsRet)
{
    ...
    if (!streamConfig.good()){
        // Create empty stone.conf if it does not excist
    ...
    {
        // Don't overwrite existing settings so command line settings override stone.conf
