...
const char * const BITCOIN_CONF_FILENAME = "stonecoin.conf";
const char * const BITCOIN_PID_FILENAME = "stonecoind.pid";
...
bool LogAcceptCategory(const char* category)
{
    if (category != NULL)
    {
        ...
        if (ptrCategory.get() == NULL)
        {
            ...
            // "stonecoin" is a composite category enabling all StoneCoin-related debug output
            if(ptrCategory->count(string("stonecoin"))) {
...
#else
    const char* pszModule = "stonecoin";
#endif
    if (pex)
        return strprintf(
...
boost::filesystem::path GetDefaultDataDir()
{
    ...
    // Windows < Vista: C:\Documents and Settings\Username\Application Data\StoneCrypto
    // Windows >= Vista: C:\Users\Username\AppData\Roaming\StoneCrypto
    // Mac: ~/Library/Application Support/StoneCrypto
    // Unix: ~/.stonecrypto
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "StoneCrypto";
#else
    fs::path pathRet;
...
#ifdef MAC_OSX
    // Mac
    return pathRet / "Library/Application Support/StoneCrypto";
#else
    // Unix
    return pathRet / ".stonecrypto";
#endif
#endif
}
                    ...
                    map<string, vector<string> >& mapMultiSettingsRet)
{
    ...
    if (!streamConfig.good()){
        // Create empty stonecoin.conf if it does not excist
    ...
    {
        // Don't overwrite existing settings so command line settings override stonecoin.conf
