...
*Setup instructions for a Gitian build of Stone Core using a Debian VM or physical system.*
...
- [Building Stone Core](#building-stone-core)
...
Clone the git repositories for Stone Core and Gitian.

```bash
git clone https://github.com/devrandom/gitian-builder.git
git clone https://github.com/stone/stone
```
...
in the Stone Core repository under 'Fetch and build inputs' to install sources which require
...
To build Stone Core (for Linux, OS X and Windows) just follow the steps under 'perform
Gitian builds' in [doc/release-process.md](release-process.md#perform-gitian-builds) in the Stone Core repository.
...
```bash
    Initialized empty Git repository in /home/debian/gitian-builder/inputs/stone/.git/
    remote: Counting objects: 57959, done.
    remote: Total 57959 (delta 0), reused 0 (delta 0), pack-reused 57958
    Receiving objects: 100% (57959/57959), 53.76 MiB | 484.00 KiB/s, done.
    Resolving deltas: 100% (41590/41590), done.
    From https://github.com/stone/stone
...
For example:
```bash
URL=https://github.com/crowning-/stone.git
COMMIT=b616fb8ef0d49a919b72b0388b091aaec5849b96
./bin/gbuild --commit stone=${COMMIT} --url stone=${URL} ../stone/contrib/gitian-descriptors/gitian-linux.yml
./bin/gbuild --commit stone=${COMMIT} --url stone=${URL} ../stone/contrib/gitian-descriptors/gitian-win.yml
./bin/gbuild --commit stone=${COMMIT} --url stone=${URL} ../stone/contrib/gitian-descriptors/gitian-osx.yml
```
...
and the stone git repository with the desired tag must both be available locally, and then gbuild must be
...
LXC_ARCH=amd64 LXC_SUITE=precise on-target -u root \
  ...
  $( sed -ne '/^packages:/,/[^-] .*/ {/^- .*/{s/"//g;s/- //;p}}' ../stone/contrib/gitian-descriptors/*|sort|uniq )
...
BTCPATH=/some/root/path/stone.git
SIGPATH=/some/root/path/stone-detached-sigs.git

./bin/gbuild --url stone=${BTCPATH},signature=${SIGPATH} ../stone/contrib/gitian-descriptors/gitian-win-signer.yml
```
...
```bash
    gpg --detach-sign ${VERSION}-linux/${SIGNER}/stone-linux-build.assert
    gpg --detach-sign ${VERSION}-win/${SIGNER}/stone-win-build.assert
    gpg --detach-sign ${VERSION}-osx-unsigned/${SIGNER}/stone-osx-build.assert
```
...
[stone/gitian.sigs](https://github.com/stone/gitian.sigs/) repository, or if that's not possible to create a pull
request.
