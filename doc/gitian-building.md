...
*Setup instructions for a Gitian build of StoneCoin Core using a Debian VM or physical system.*

Gitian is the deterministic build process that is used to build the StoneCoin
...
- [Building StoneCoin Core](#building-stonecoin-core)
...
Clone the git repositories for StoneCoin Core and Gitian.

```bash
git clone https://github.com/devrandom/gitian-builder.git
git clone https://github.com/stonecrypto/stonecoin
```
...
in the StoneCoin Core repository under 'Fetch and build inputs' to install sources which require
manual intervention. Also optionally follow the next step: 'Seed the Gitian sources cache
and offline git repositories' which will fetch the remaining files required for building
offline.

Building StoneCoin Core
----------------

To build StoneCoin Core (for Linux, OS X and Windows) just follow the steps under 'perform
Gitian builds' in [doc/release-process.md](release-process.md#perform-gitian-builds) in the StoneCoin Core repository.
...
```bash
    Initialized empty Git repository in /home/debian/gitian-builder/inputs/stonecoin/.git/
    remote: Counting objects: 57959, done.
    remote: Total 57959 (delta 0), reused 0 (delta 0), pack-reused 57958
    Receiving objects: 100% (57959/57959), 53.76 MiB | 484.00 KiB/s, done.
    Resolving deltas: 100% (41590/41590), done.
    From https://github.com/stonecrypto/stonecoin
...
For example:
```bash
URL=https://github.com/crowning-/stonecoin.git
COMMIT=b616fb8ef0d49a919b72b0388b091aaec5849b96
./bin/gbuild --commit stonecoin=${COMMIT} --url stonecoin=${URL} ../stonecoin/contrib/gitian-descriptors/gitian-linux.yml
./bin/gbuild --commit stonecoin=${COMMIT} --url stonecoin=${URL} ../stonecoin/contrib/gitian-descriptors/gitian-win.yml
./bin/gbuild --commit stonecoin=${COMMIT} --url stonecoin=${URL} ../stonecoin/contrib/gitian-descriptors/gitian-osx.yml
```
...
and the stonecoin git repository with the desired tag must both be available locally, and then gbuild must be
...
LXC_ARCH=amd64 LXC_SUITE=precise on-target -u root \
  ...
  $( sed -ne '/^packages:/,/[^-] .*/ {/^- .*/{s/"//g;s/- //;p}}' ../stonecoin/contrib/gitian-descriptors/*|sort|uniq )
...
BTCPATH=/some/root/path/stonecoin.git
SIGPATH=/some/root/path/stonecoin-detached-sigs.git

./bin/gbuild --url stonecoin=${BTCPATH},signature=${SIGPATH} ../stonecoin/contrib/gitian-descriptors/gitian-win-signer.yml
```
...
```bash
    gpg --detach-sign ${VERSION}-linux/${SIGNER}/stonecoin-linux-build.assert
    gpg --detach-sign ${VERSION}-win/${SIGNER}/stonecoin-win-build.assert
    gpg --detach-sign ${VERSION}-osx-unsigned/${SIGNER}/stonecoin-osx-build.assert
```
...
[stonecoin/gitian.sigs](https://github.com/stonecrypto/gitian.sigs/) repository, or if that's not possible to create a pull
request.
